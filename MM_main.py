...
# Needs serial which comes from pyserial version 2.5
...
def songBrowser(win):    # Open browser for accompaniment songfiles
    ...
    dlg = wx.FileDialog(None, "Open a song file", style = wx.OPEN)
...
for deviceID in range(deviceCount):
    ...
    #print device
    if device[1][-5:] == 'SD-50' and device[3] == 1:   #device can be "3- SD-50" or "SD-50" depending how connected
        midout = pygame.midi.Output(deviceID)
        roland = True
        rolandOK.SetLabel("Roland OK")
    if device[1] == 'eobody 2' and device[2] == 1:
